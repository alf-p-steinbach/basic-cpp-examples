...
void draw_c_curve(
    box_drawing::Canvas&        a_canvas,
    const Position&             a_start_position,
    const Direction::Enum       a_direction,
    const int                   a_level
    )
{
    struct Retained_state
    {
        box_drawing::Canvas&    canvas;
        Position                where;

        void draw( const Direction::Enum direction, const int level )
        {
            if( level == 0 ) {
                where = canvas.unit_line( where, direction );   // Returns line end point.
            } else {
                draw( direction, level - 1 );
                draw( left_from( direction ), level - 1 );
            }
        }
    };  // Retained_state

    Retained_state{ a_canvas, a_start_position }.draw( a_direction, a_level );
};
...
auto main() -> int
{
    auto canvas = box_drawing::Canvas({40 + 20, 64});

    draw_c_curve( canvas, {31, 15}, Direction::left, 10 );
    draw_plus_sign( canvas, {34, 21}, 7 );
    draw_plus_sign( canvas, {42, 20}, 7 );
    out << canvas.widened().to_string() << endl;   // "widened": 2x width for console display.
}
